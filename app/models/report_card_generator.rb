class ReportCardGenerator < ApplicationRecord
        ...
        if first_seq.present? && second_seq.present?
          if student.was_enrolled?(first_seq.hashed_marks) && student.was_enrolled?(second_seq.hashed_marks)
            ...
            [first_seq, second_seq].each do |s|
              @warning_messages << generate_warning_message("Missing enrollment for student #{student.full_name}", s) unless student.was_enrolled?(s.hashed_marks)
            end
          end
        else
          @failed_errors << { "title" => error_name_list("missing_seq")["message"], "class_name" => subject.class.name, "id" => subject.id }
        end
      end
  ...
  def self.generate_warning_message(title, object)
    { "title" => title, "class_name" => object.class.name, "id" => object.id }
  end
  ...
  def self.check_sequences_status
    ...
    unless sequences.all? { |s| s.approved? }
      ...
      unapproved_seq.each do |s|
        @failed_errors << { "title" => error_name_list("unapproved_seq")["message"], "class_name" => s.class.name, "id" => s.id }
      end
    end
  end
